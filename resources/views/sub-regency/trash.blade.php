...
@section('title', 'Kecamatan - Recycle Bin')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0">
            <i class="bi bi-trash"></i> Kecamatan - Recycle Bin
        </h1>
    ...
    <div class="card">
        <div class="card-header bg-danger text-white">
            <h5 class="card-title mb-0">
                <i class="bi bi-archive"></i> Data Kecamatan yang Dihapus
            </h5>
        </div>
        <div class="card-body">
            @if($subRegencies->count() > 0)
                <div class="table-responsive">
                    <table class="table table-striped table-hover" id="subRegencyTrashTable">
                        ...
                        <tbody>
                            @foreach($subRegencies as $key => $subRegency)
                                <tr>
                                    ...
                                    <td>{{ $subRegency->regency->type }} {{ $subRegency->regency->name }}</td>
                                    <td>{{ $subRegency->regency->province->name }}</td>
                                    <td>{{ $subRegency->deleted_by ?? '-' }}</td>
                                    <td>
                                        @if($subRegency->deleted_at)
                                            {{ $subRegency->deleted_at->format('d/m/Y') }}
                                            <br>
                                            <small class="text-muted">{{ $subRegency->deleted_at->format('H:i') }}</small>
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>
                                        <div class="btn-group" role="group">
                                            <button type="button" 
                                                    ...
                                                    data-id="{{ $subRegency->id }}"
                                                    data-name="{{ $subRegency->name }}"
                                                    title="Restore">
            ...
            @else
                <div class="alert alert-info mb-0">
                    <i class="bi bi-info-circle"></i> Tidak ada data kecamatan di tempat sampah.
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Anda akan menghapus kecamatan: <strong id="subRegencyNameForceDelete"></strong></p>
...
$(document).ready(function() {
    ...
    @if($subRegencies->count() > 0)
    $('#subRegencyTrashTable').DataTable({
        language: {
    ...
    $('.restore-btn').on('click', function() {
        const subRegencyId = $(this).data('id');
        const subRegencyName = $(this).data('name');
        
        Swal.fire({
            title: 'Restore Kecamatan',
            html: `Apakah Anda yakin ingin memulihkan kecamatan:<br><strong>${subRegencyName}</strong>?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/sub-regency/${subRegencyId}/restore`,
                    type: 'PATCH',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Gagal memulihkan kecamatan',
                            icon: 'error',
    ...
    $('.force-delete-btn').on('click', function() {
        ...
        $('#subRegencyNameForceDelete').text(subRegencyName);
        $('#forceDeleteForm').attr('action', `/sub-regency/${subRegencyId}/force-delete`);
    ...
    $('#forceDeleteForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                Swal.fire({
                    title: 'Error!',
                    text: xhr.responseJSON?.message || 'Gagal menghapus kecamatan',
                    icon: 'error',
                    confirmButtonColor: '#dc3545'
                });
            }
