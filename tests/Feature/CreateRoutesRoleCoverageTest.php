...
class CreateRoutesRoleCoverageTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_all_create_routes_work_for_allowed_roles(): void
    {
        $routes = app('router')->getRoutes()->getRoutes();

        $createRoutes = array_values(array_filter($routes, function ($route) {
            $name = $route->getName();
            if (!$name) {
                return false;
            }
        ...
        $this->assertNotEmpty($createRoutes, 'No create routes found to audit.');
        ...
        foreach ($createRoutes as $route) {
            ...
            if (!$requiresAuth) {
                ...
                $response->assertStatus(200, "Public create route failed: {$name}");
                continue;
            }

            $guest = $this->get(route($name));
            $guest->assertStatus(302, "Auth-protected create did not redirect for guest: {$name}");
            ...
            foreach ($rolesToTest as $roleName) {
                ...
                if ($bufferLevelAfter > $bufferLevelBefore) {
                    ...
                    $this->fail("Create route leaked output buffer(s): {$name} (role={$roleName})");
                }

                if ($response->getStatusCode() === 500 && $response->exception) {
                    $this->fail("Create route exception: {$name} (role={$roleName}) - " . $response->exception->getMessage());
                }

                $this->assertTrue(
                    in_array($response->getStatusCode(), [200, 302, 303], true),
                    "Create route failed: {$name} (role={$roleName}, status={$response->getStatusCode()})"
                );
