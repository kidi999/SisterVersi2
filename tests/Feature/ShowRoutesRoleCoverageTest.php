...
class ShowRoutesRoleCoverageTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function seedDatabaseOnce(): void
    {
        ...
        // Seed full dataset so most models used in show routes have at least 1 row.
        $this->seed();
        $this->seeded = true;
    }
    ...
    private function inferModelClassForShowRoute($route, string $paramName): ?string
    {
        ...
        // Fallback: infer from route param name (e.g. jadwalKuliah -> App\Models\JadwalKuliah)
    ...
    public function test_all_show_routes_work_for_allowed_roles_when_data_exists(): void
    {
        $this->seedDatabaseOnce();

        $routes = app('router')->getRoutes()->getRoutes();

        $showRoutes = array_values(array_filter($routes, function ($route) {
            $name = $route->getName();
            if (!$name) {
                return false;
            }

            if (!str_ends_with((string) $name, '.show')) {
                return false;
            }
            ...
            // We only handle simple show routes with a single parameter.
            return count($route->parameterNames()) === 1;
        }));

        $this->assertNotEmpty($showRoutes, 'No show routes found to audit.');
        ...
        foreach ($showRoutes as $route) {
            ...
            $modelClass = $this->inferModelClassForShowRoute($route, $paramName);

            if (!$modelClass) {
                // Can't infer record for this show route reliably.
                continue;
            }

            $record = $this->resolveRouteParamValue($modelClass);
            if (!$record) {
                // No data for this model in seeded DB; skip.
                continue;
            }
            ...
            if (!$requiresAuth) {
                ...
                $response->assertStatus(200, "Public show route failed: {$name}");
                continue;
            }

            $guest = $this->get($url);
            $guest->assertStatus(302, "Auth-protected show did not redirect for guest: {$name}");
            ...
            foreach ($rolesToTest as $roleName) {
                ...
                if ($bufferLevelAfter > $bufferLevelBefore) {
                    ...
                    $this->fail("Show route leaked output buffer(s): {$name} (role={$roleName})");
                }

                if ($response->getStatusCode() === 500 && $response->exception) {
                    $this->fail("Show route exception: {$name} (role={$roleName}) - " . $response->exception->getMessage());
                }

                // Some show routes may redirect based on policy/data; accept 200 or redirect.
                $this->assertTrue(
                    in_array($response->getStatusCode(), [200, 302, 303, 403], true),
                    "Show route failed: {$name} (role={$roleName}, status={$response->getStatusCode()})"
                );
